...
class MealProvider {
  ...
  static List<Meal> getMealsInCategory(String categoryId, [List<Meal>? mealCollection]) {
    ...
    return toBeChecked.where((meal) => meal.categories.contains(categoryId)).toList();
  }

  /// Filters meals based on the currently applied filters.
  static List<Meal> getFilteredMeals(WidgetRef ref, [List<Meal>? mealCollection]) {
    List<Meal> toBeChecked = mealCollection ?? meals;
    ...
    return toBeChecked.where((meal) {
      return ref.read(filterProvider.notifier).passesFilter(meal.mealFilterProperties);
    }).toList();
  }

  /// Wrapper around [getMealsInCategory] and [getFilteredMeals].
  static List<Meal> getFilteredMealsInCategory(WidgetRef ref, String categoryId, [List<Meal>? mealCollection]) {
    List<Meal> toBeChecked = mealCollection ?? meals;
